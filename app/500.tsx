"use client"

import Link from "next/link"
import { FuzzyText } from "@/components/fuzzy-text"
import { Home, RefreshCw, Server } from "lucide-react"

export default function ServerError() {
  return (
    <div className="min-h-screen flex items-center justify-center px-4 relative overflow-hidden">
      {/* Background gradient */}
      <div className="absolute inset-0 bg-gradient-to-br from-background via-background to-orange-500/5" />
      
      {/* Animated background elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-orange-500/10 rounded-full blur-3xl animate-pulse" />
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-orange-500/5 rounded-full blur-3xl animate-pulse delay-1000" />
      </div>

      <div className="relative z-10 text-center max-w-2xl mx-auto">
        {/* FuzzyText 500 */}
        <div className="mb-8 flex justify-center">
          <FuzzyText
            text="500"
            fontSize={140}
            fontWeight={900}
            color="#f97316"
            enableHover={true}
            baseIntensity={0.18}
            hoverIntensity={0.5}
          />
        </div>

        {/* Server icon with animation */}
        <div className="mb-6 flex justify-center">
          <div className="relative">
            <div className="absolute inset-0 bg-orange-500/20 rounded-full blur-xl animate-pulse" />
            <div className="relative bg-orange-500/10 p-4 rounded-full">
              <Server className="w-12 h-12 text-orange-500" />
            </div>
          </div>
        </div>

        {/* Error message */}
        <h1 className="text-3xl md:text-4xl font-bold mb-4 text-foreground">
          Internal Server Error
        </h1>
        <p className="text-lg text-muted-foreground mb-8 leading-relaxed">
          Oops! Something went wrong on our end. Our servers are having a moment.
          <br />
          Don't worry, we're on it! Please try again in a few moments.
        </p>

        {/* Action buttons */}
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <button
            onClick={() => window.location.reload()}
            className="inline-flex items-center justify-center gap-2 px-8 py-3 bg-orange-600 hover:bg-orange-700 text-white font-medium rounded-lg transition-all duration-200 text-lg"
          >
            <RefreshCw className="w-5 h-5" />
            Reload Page
          </button>
          <Link
            href="/"
            className="inline-flex items-center justify-center gap-2 px-8 py-3 glass border border-border hover:border-orange-500/50 text-white font-medium rounded-lg transition-all duration-200 text-lg"
          >
            <Home className="w-5 h-5" />
            Go Home
          </Link>
        </div>

        {/* Help text */}
        <div className="mt-12 pt-8 border-t border-border/50">
          <p className="text-sm text-muted-foreground mb-4">
            If this problem persists, please try:
          </p>
          <div className="flex flex-wrap gap-3 justify-center text-sm">
            <button
              onClick={() => window.location.reload()}
              className="text-orange-500 hover:underline"
            >
              Refresh the page
            </button>
            <span className="text-muted-foreground">•</span>
            <button
              onClick={() => {
                localStorage.clear()
                sessionStorage.clear()
                window.location.reload()
              }}
              className="text-orange-500 hover:underline"
            >
              Clear cache
            </button>
            <span className="text-muted-foreground">•</span>
            <Link href="/#contact" className="text-orange-500 hover:underline">
              Contact support
            </Link>
          </div>
        </div>
      </div>
    </div>
  )
}
